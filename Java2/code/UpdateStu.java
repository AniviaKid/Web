    ...
    public static void main(String args[]) {
        ...
        while ((lineWords=getNextLineWords())!=null){
            ...
            for(int i=0;i<ins.length();i++){
                if(ins.charAt(i)==' ') num_space++;
            }
            if(num_space==3){ //id num name
                String id=new String(),num=new String(),name=new String();
                int i=0;
                for(;i<ins.length();i++){
                    if(ins.charAt(i)==' '){
                        i++;
                ...
                for(;i<ins.length();i++) name+=ins.charAt(i);
                if (connect()) {
                    int rs = executeUpdate("update stu set num='"+num+"',name='"+name+"' where id="+id+";");
                    System.out.println(rs+"条记录被修改");
                } else {
                    System.out.println("Connect Error!");
                }
            }
            else if(num_space==2){ //num name
                ...
                for(;i<ins.length();i++) name+=ins.charAt(i);
                if (connect()) {
                    int rs = executeUpdate("update stu set name='"+name+"' where num='"+num+"';");
                    System.out.println(rs+"条记录被修改");
                } else {
                    System.out.println("Connect Error!");
                }
